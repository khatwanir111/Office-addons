...
OUTPUT = "team_creation_dates.csv"
...
def main():
    ...
    teams = requests.get(
        "https://graph.microsoft.com/v1.0/groups?$filter=resourceProvisioningOptions/Any(x:x eq 'Team')&$select=displayName,createdDateTime",
        headers=headers
    ).json()["value"]
